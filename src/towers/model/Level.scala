...
//        )
        ...
        for(multiple <- 1 until 100){
          startingLocation = startingLocation :+ new GridLocation(start + rnd.nextInt( (end - start) + 1 ), startY + rnd.nextInt( (endY - startY) + 1 ))
        }
      }
    }else if(number == 1) {
      new Level {
...
class Level {
  ...
  var foodLocations:List[GridLocation] = List()

  var gridWidth: Int = 41
  var gridHeight: Int = 20

  var startingLocation: List[GridLocation] = List()
