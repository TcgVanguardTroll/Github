...
function parseGameState(event) {
    ...
    // placeCircle(gameState['start']['x'], gameState['start']['y'], '#f47142',2.0);
    ...
    // placeCircle(gameState['base']['x'], gameState['base']['y'], color,1.0);
    ...
    for (let player of gameState['players']) {
        placeCircle(player['x'], player['y'], player['id'] === socket.id ? '#ffff00' : '#56bcff', player["radius"]);
    }

    for (let food of gameState['foods']) {
        placeCircle(food['x'], food['y'], 'red', 1.0);
    }
